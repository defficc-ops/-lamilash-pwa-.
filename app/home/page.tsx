  ...
  return (
    <div className="page-container bg-cream">
      ...
      >
        ...
        <div className="relative z-10 px-6 pt-10 pb-6">
          ...
          >
            <ScrollReveal direction="up" delay={0.2} blur>
              <Link href="/booking" className="btn-primary flex-1 py-4 text-xs uppercase tracking-widest flex items-center justify-center gap-2 hover-slow">
                <Calendar size={14} />
                Записаться
              </Link>
            </ScrollReveal>
            <ScrollReveal direction="up" delay={0.3} blur>
              <Link href="/portfolio" className="glass-premium text-espresso px-5 py-4 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-gold/10 transition-all flex items-center justify-center hover-slow">
                Работы
              </Link>
            </ScrollReveal>
          </motion.div>
        ...
        <ScrollReveal direction="up" distance={50} duration={1.5} blur>
      ...
      <section className="px-5 mt-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          {SERVICES_PREVIEW.map((svc, i) => (
            <ScrollReveal direction="up" delay={i * 0.1} key={svc.name} blur>
              <Link href={svc.href} id={`service-card-${i}`}>
                <div className="service-card p-4 hover-slow">
                  <span className="text-2xl block mb-2">{svc.icon}</span>
                  <p className="font-semibold text-espresso text-[13px] leading-snug mb-2 text-balance">{svc.name}</p>
      ...
      <section className="px-5 mt-10">
        <ScrollReveal direction="up" blur>
          <h2 className="heading-premium text-2xl text-espresso mb-5">
        ...
        <div className="space-y-3">
          {[
            ...
            <ScrollReveal direction="up" delay={i * 0.15} key={title} blur>
              <div className="flex gap-4 glass-card rounded-2xl p-4 hover-slow">
      ...
      <section className="px-5 mt-12">
        <ScrollReveal direction="up" blur>
          <div className="flex items-center justify-between mb-5">
            <h2 className="heading-premium text-2xl text-espresso">Мои работы</h2>
        ...
        <div className="scroll-x pb-4">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((i) => (
            <ScrollReveal direction="left" delay={i * 0.1} key={i} distance={20} blur={false}>
              <div className="flex-shrink-0 w-48 h-64 rounded-2xl overflow-hidden glass-card border-white/40 relative group cursor-pointer hover-slow">
                <img 
                  src={`/images/work${i}.png`} 
                  alt={`Результат ${i}`}
                  className="w-full h-full object-cover transition-all duration-[1.5s] ease-[0.22,1,0.36,1] group-hover:scale-110 group-hover:blur-[2px]"
                  onError={(e) => {
                    (e.target as any).src = 'https://images.unsplash.com/photo-1512496015851-a90fb38ba796?auto=format&fit=crop&q=80&w=400';
                  }}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-espresso/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex items-end p-5">
                   <p className="text-white text-xs font-medium backdrop-blur-md bg-white/20 border border-white/30 px-4 py-2 rounded-full shadow-lg">Посмотреть ✧</p>
                </div>
              </div>
            </ScrollReveal>
          ))}
